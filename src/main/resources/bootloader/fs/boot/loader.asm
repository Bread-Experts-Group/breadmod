...
; Section Header Reading
	movzx ecx, WORD [esi + 0x30] ; entry count
	mov edx, DWORD [esi + 0x20] ; first entry offset in file
	add edx, esi
pushsee:push DWORD [edx + 0x04] ; type
nextsee:add dx, WORD [esi + 0x2E]
	loop pushsee
	hlt
bits 16
;;;;;;;;;;;;;;;;;;;;;;;;; GDT
...
int10_msg_read_fail   : db "cdr read failure", 0x0A, 0
