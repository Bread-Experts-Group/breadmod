...
class FluidContainer(val tanks: MutableMap<FluidTank, StorageDirection>) : IFluidHandler,
    ...
    var insertFluidCheck: ((resource: FluidStack, action: IFluidHandler.FluidAction) -> Boolean)? = { _, _ -> true }
    var extractFluidCheck: ((resource: FluidStack, action: IFluidHandler.FluidAction) -> Boolean)? = { _, _ -> true }
    var extractFluidTagCheck: ((resource: TagKey<Fluid>, amount: Int, action: IFluidHandler.FluidAction) -> Boolean)? =
        { _, _, _ -> true }


    override fun fill(resource: FluidStack, action: IFluidHandler.FluidAction): Int {
        if (resource.isEmpty || insertFluidCheck?.invoke(resource, action) != true) return 0
    ...
    override fun drain(resource: FluidStack, action: IFluidHandler.FluidAction): FluidStack {
        if (resource.isEmpty || extractFluidCheck?.invoke(
                resource,
                action
            ) != true
    ...
    fun drain(resource: TagKey<Fluid>, amount: Int, action: IFluidHandler.FluidAction): FluidStack {
    ...
    override fun drain(maxDrain: Int, action: IFluidHandler.FluidAction): FluidStack {
        return drain(tanksWithDirection(StorageDirection.EMPTY_ONLY).firstOrNull { !it.isEmpty }?.fluid!!, action)

//        return drain(tanksWithDirection(StorageDirection.EMPTY_ONLY).firstOrNull { !it.isEmpty }?.fluid?.let {
//            FluidStack(
//                it.fluid,
//                maxDrain
//            )
//        }, action)
    ...
    override fun serializeNBT(provider: HolderLookup.Provider): CompoundTag = CompoundTag().also {
        ...
        tanks.forEach { (tank, direction) ->
            it.put(
                index.toString(),
                CompoundTag().also { tank.writeToNBT(provider, it); it.putString("TransitDirection", direction.name) })
            index++
        }
    }
    ...
    override fun deserializeNBT(provider: HolderLookup.Provider, nbt: CompoundTag) {
        ...
        tanks.forEach { (tank, _) ->
            // Probably not deterministic... oh well
            tank.readFromNBT(provider, nbt.getCompound(index.toString()))
            index++
        }
    }

    companion object {
        fun FluidTank.drain(resource: TagKey<Fluid>, amount: Int, action: IFluidHandler.FluidAction): FluidStack {
