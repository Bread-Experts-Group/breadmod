...
class InternetChatRelayOverlay : LayeredDraw.Layer {
	private var setDelta: Boolean = false // Hacky solution to reset the delta with animation progress
	...
	private var animationDelta: Double = 0.0
		set(value) {
			field = clamp(value, 0.0, 3.0)
		}
	private var shouldShow: Boolean = false // Should only be true when the ChatScreen is active
	private var showEdge: Boolean = false // Should only be true when an IRC connection is active
	private val texture: ResourceLocation = modLocation("textures", "gui", "hud", "irc_overlay.png")
