...
class SingleFluidItemRecipe(
    ...
    class Serializer : BMRecipeSerializer<SingleFluidItemRecipe>() {
        override fun codec(): MapCodec<SingleFluidItemRecipe> = RecordCodecBuilder.mapCodec { inst ->
            inst.group(
                ...
                optionalIntCodecModule("time", SingleFluidItemRecipe::rTime),
                optionalIntCodecModule("energy", SingleFluidItemRecipe::rEnergy)
            ).apply(inst, ::SingleFluidItemRecipe)
        }

        override fun streamCodec(): StreamCodec<RegistryFriendlyByteBuf, SingleFluidItemRecipe> = StreamCodec.composite(
    ...
    class Builder(
        ...
        override fun getResult(): Item = itemResult.first
        fun getFluid(): Fluid = fluidResult.first

        override fun save(recipeOutput: RecipeOutput, id: ResourceLocation) {
            val recipe = SingleFluidItemRecipe(
                fluid,
                item,
                ItemStack(itemResult.first).copyWithCount(itemResult.second),
                FluidStack(fluidResult.first, fluidResult.second),
                time,
                energy
            )
