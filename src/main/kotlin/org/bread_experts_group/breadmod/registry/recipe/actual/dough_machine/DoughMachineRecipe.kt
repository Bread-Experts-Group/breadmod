...
class DoughMachineRecipe(
	...
	override fun matches(input : DoughMachineInput, level : Level) : Boolean =
		Ingredient.of(this.recipeInput).test(input.input) && FluidIngredient.of(this.recipeFluidInput)
			.test(input.fluidInput) &&
				this.recipeTime >= 0 && this.recipeEnergy >= 0
	...
	override fun getResultItem(registries : HolderLookup.Provider) : ItemStack = this.recipeOutput
	...
	class DoughMachineInput(
		...
		override fun getItem(index : Int) : ItemStack =
			if (index == 0) this.input else ItemStack.EMPTY
