	...
	override fun matches(input: CraftingInput, level: Level): Boolean {
		...
		input.items().forEach {
			if (!it.isEmpty) {
				...
				else if (it.`is`(this.toolTag) && !hasTool) hasTool = true
				else return false
			}
		}
			...
			input.getItem(slot).also { stack ->
				if (!stack.isEmpty)
					if (stack.`is`(this.input.item))
						if (((itemToSplit.size + 1) * this.outputMultiplier) <= this.output.maxStackSize) {
							itemToSplit.add(stack)
						} else shouldBreak = true
					else if (stack.`is`(this.toolTag)) {
