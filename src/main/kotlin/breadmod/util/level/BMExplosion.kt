    ...
    override fun explode() {
        if (pLevel is ServerLevel) {
            ...
            consider.forEach {
                ...
                DAMAGE_CALC.getBlockExplosionResistance(this, pLevel, blockPos, state, pLevel.getFluidState(it))
                    .ifPresent { resist ->
                        if ((pRadius * (0.7 + (pLevel.random.nextFloat() * 0.6)) - ((resist + 0.3) * 0.3)) > 0) toDestroy.add(
                            it to state
                        )
                    }
                println(
                    "block explosion resistance: ${state.block}, ${
                        state.block.getExplosionResistance(
                            state,
                            pLevel,
                            blockPos,
                            this
                ...
                if (float > 0.0F && DAMAGE_CALC.shouldBlockExplode(this, pLevel, blockPos, state, float)) toDestroy.add(
                    it to state
                )
...
//            val blockDrops = mutableListOf<Pair<BlockPos, ItemStack>>()
            toDestroy.forEach { (pos, state) ->
                if (!state.isAir) {
                    ...
                    if (pFire != null && (pLevel.random.nextInt() * pFire == 0) && !pLevel.getBlockState(pos.below())
                            .isSolidRender(pLevel, pos)
                    )
    ...
    fun explodeThreaded() {
        ...
        if (!pLevel.isClientSide) {
            pLevel.playSound(
                null,
                BlockPos(pPos.x.toInt(), pPos.y.toInt(), pPos.z.toInt()),
