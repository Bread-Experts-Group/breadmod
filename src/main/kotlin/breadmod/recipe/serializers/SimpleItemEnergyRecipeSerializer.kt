...
class SimpleItemEnergyRecipeSerializer<T: WheatCrusherRecipe>(
    val factory: (
        cId: ResourceLocation, cTime: Int, cEnergy: Int?,
        cItemsRequired: List<ItemStack>?,
        cItemTagsRequired: List<Pair<TagKey<Item>, Int>>?,
        cItemsOutput: List<ItemStack>?
    ) -> T
) : RecipeSerializer<T> {
    companion object {
    ...
    override fun fromNetwork(p0: ResourceLocation, p1: FriendlyByteBuf): T =
        factory(
            ...
            p1.readNullable { p1.readItemList () },
            p1.readNullable { p1.readTagList(ForgeRegistries.ITEMS) },
            p1.readNullable { p1.readItemList() }
        )
